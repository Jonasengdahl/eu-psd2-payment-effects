# scripts/17_baseline_did.R
# ============================================================
# Baseline DiD estimation (counts only; CT vs Card; no controls)
#
# Purpose (Stage 2):
#   - Estimate baseline TWFE DiD with EU treated vs US/CA controls
...
#
# Outputs:
#   - outputs/tables/tab17_baseline_did_counts_raw.csv
#   - outputs/tables/tab17_baseline_did_counts_latex.tex  (LaTeX fragment)
#   - outputs/figures/Fig17_baseline_did_beta.png         (optional coef plot)
# ============================================================
...
out_raw <- here::here("outputs/tables/tab17_baseline_did_counts_raw.csv")
out_tex <- here::here("outputs/tables/tab17_baseline_did_counts_latex.tex")
out_fig <- here::here("outputs/figures/Fig17_baseline_did_beta.png")
...
controls <- c("US", "CA")  # adjust only if your BIS codes differ
...
# Reproducibility (fwildclusterboot also recommends setting dqrng seed)
set.seed(123)
...
wild_pval <- function(model) {
  ...
  # Different versions store p-values under slightly different names
...
# ---- 8) Create LaTeX fragment for Table 1 (Panel A + key stats) ---------
...
latex_lines <- c(
  "% Auto-generated by scripts/17_baseline_did.R",
  "% Panel A: Treatment effect + inference",
  paste0("EU $\\times$ Post(2018+) & \\textbf{", b1, "} & \\textbf{", b2, "} \\\\"),
  paste0("& (", s1, ") & (", s2, ") \\\\"),
  paste0("Wild bootstrap p-value & ", p1, " & ", p2, " \\\\"),
  "\\midrule",
  "% Panel B: key sample stats (optional to include here; can also be hard-coded in main TeX)",
...
p <- ggplot2::ggplot(plot_df, ggplot2::aes(x = outcome_label, y = beta)) +
  ...
  ggplot2::labs(
    title = "Baseline DiD estimates (EU × Post)",
    subtitle = paste0(
      "Counts-based outcomes; clustered SEs with 95% CI. ",
      "Wild bootstrap p-values in Table 1. (B = ", B_boot, ")"
    ),
    x = NULL,
...
message("17_baseline_did.R complete.")
