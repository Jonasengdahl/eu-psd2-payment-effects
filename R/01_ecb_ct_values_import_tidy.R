# scripts/01_ecb_ct_values_import_tidy.R
# =========================================
# Import & tidy ECB credit transfer VALUES (EU27, annual) — WIDE EXPORT
# Objective:
#   - Read raw ECB credit transfer transactions (sent, values) in WIDE format
...
#
# Inputs:
#   data/raw/ecb/credit_transfers/values/ECB_ct_sent_values_EU27_annual_raw.csv
#
# Outputs:
#   data/intermediate/ecb/credit_transfers/ct_values_country_year_intermediate.csv
#   data/intermediate/ecb/credit_transfers/ct_values_country_year_intermediate.rds
#   outputs/tables/ct_values_missingness_by_country.csv
#   outputs/tables/ct_values_country_coverage.csv
#   outputs/tables/ct_values_series_metadata.txt
#   outputs/logs/01_ecb_ct_values_import_tidy_log.txt
# =========================================
...
# 0. Logging
# =========================================
log_file <- here::here("outputs/logs", "01_ecb_ct_values_import_tidy_log.txt")
...
cat("\n=========================================\n")
cat("01_ecb_ct_values_import_tidy.R — START\n")
...
# =========================================
raw_file <- here::here(
  "data/raw/ecb/credit_transfers/values",
  "ECB_ct_sent_values_EU27_annual_raw.csv"
)
...
cat("  ", out_csv, "\n")
cat("  ", out_rds, "\n")
cat("  ", here::here("outputs/tables", "ct_values_missingness_by_country.csv"), "\n")
cat("  ", here::here("outputs/tables", "ct_values_country_coverage.csv"), "\n")
cat("  ", here::here("outputs/tables", "ct_values_series_metadata.txt"), "\n")
cat("  ", log_file, "\n\n")
...
# Filter 3 (recommended if present): keep VN unit marker for VALUES
# (ECB often uses VN for value series; keep this only if it appears in column names)
if (any(stringr::str_detect(series_cols, "\\.VN\\)"))) {
  series_cols <- series_cols[stringr::str_detect(series_cols, "\\.VN\\)")]
} else if (any(stringr::str_detect(series_cols, "\\.VN$"))) {
  series_cols <- series_cols[stringr::str_detect(series_cols, "\\.VN$")]
}
...
writeLines(meta_lines, here::here("outputs/tables", "ct_values_series_metadata.txt"))
...
ct_long <- ct_long |>
  dplyr::mutate(
    ...
    value_raw = dplyr::na_if(value_raw, ":"),
    ct_sent_value = readr::parse_number(
      value_raw,
      locale = readr::locale(decimal_mark = ".", grouping_mark = ",")
    )
  )

cat("Share missing (NA) in ct_sent_value: ",
    round(mean(is.na(ct_long$ct_sent_value)), 4), "\n\n")
...
ct_panel <- ct_long |>
  ...
  dplyr::summarise(
    ct_sent_value = dplyr::first(ct_sent_value),
    n_rows = dplyr::n(),
    .groups = "drop"
  )
...
readr::write_csv(miss_by_country, file.path(diag_dir, "ct_values_missingness_by_country.csv"))
readr::write_csv(coverage_by_year, file.path(diag_dir, "ct_values_country_coverage.csv"))
...
# =========================================
cat("\n=========================================\n")
cat("01_ecb_ct_values_import_tidy.R — END\n")
cat("Timestamp: ", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n")
cat("=========================================\n\n")
