# scripts/06_ecb_card_values_import_tidy.R
# =========================================
# Import & tidy ECB card payments values (EU27, annual) — WIDE EXPORT
# Objective:
#   - Read raw ECB card payments (sent, values) in WIDE format
#   - Filter to intended series definition (avoid mixing multiple PCP series)
#   - Tidy into a clean country-year panel (long format)
#   - Save intermediate outputs + diagnostics + logs
#
# Inputs:
#   data/raw/ecb/card_payments/values/ECB_cp_values_EU27_annual_raw.csv
#
# Outputs:
#   data/intermediate/ecb/card_payments/cp_values_country_year_intermediate.csv
#   data/intermediate/ecb/card_payments/cp_values_country_year_intermediate.rds
#   outputs/tables/cp_values_missingness_by_country.csv
#   outputs/tables/cp_values_country_coverage.csv
#   outputs/tables/cp_values_series_metadata.txt
#   outputs/logs/06_card_values_import_tidy_log.txt
# =========================================
...
# 0. Logging
# =========================================
log_file <- here::here("outputs/logs", "06_card_values_import_tidy_log.txt")
...
cat("\n=========================================\n")
cat("06_card_values_import_tidy.R — START\n")
...
# =========================================
raw_file <- here::here(
  "data/raw/ecb/card_payments/values",
  "ECB_cp_values_EU27_annual_raw.csv"
)
...
cat("  ", out_csv, "\n")
cat("  ", out_rds, "\n")
cat("  ", here::here("outputs/tables", "cp_values_missingness_by_country.csv"), "\n")
cat("  ", here::here("outputs/tables", "cp_values_country_coverage.csv"), "\n")
cat("  ", here::here("outputs/tables", "cp_values_series_metadata.txt"), "\n")
cat("  ", log_file, "\n\n")
...
cat("Any PCP columns? ", any(stringr::str_detect(names(raw), "PCP\\.")), "\n\n")
...
# Filter 1: keep annual PCP series columns (series key, not label text)
series_cols <- series_cols[stringr::str_detect(series_cols, "PCP\\.A\\.")]

# Filter 2: keep EUR unit marker for values
...
# Filter 3: keep EUR unit marker for values (more robust)
if (any(stringr::str_detect(series_cols, "\\.EUR"))) {
  series_cols <- series_cols[stringr::str_detect(series_cols, "\\.EUR")]
}

dropped <- setdiff(series_cols_all, series_cols)
cat("Dropped series columns (n = ", length(dropped), "):\n", sep = "")
print(dropped)
cat("\n")
...
# Save a small metadata note for transparency
meta_lines <- c(
...
writeLines(meta_lines, here::here("outputs/tables", "cp_values_series_metadata.txt"))
...
cat("Number of series columns detected: ", length(series_cols), "\n\n")
cat("Long data dimensions: ", nrow(cp_long), " rows × ", ncol(cp_long), " cols\n\n")

# =========================================
# 6. Extract country ISO2 from series key (PCP.A.SE....)
# =========================================
print_section("6. Extract country code from series key")

cp_long <- cp_long |>
  dplyr::mutate(
    country = stringr::str_match(series_name, "PCP\\.A\\.([A-Z]{2})\\.")[, 2]
  )
...
cat(
  "Share missing (NA) in cp_value: ",
  round(mean(is.na(cp_long$cp_value)), 4), "\n\n"
)
...
cp_panel <- cp_long |>
  ...
  dplyr::summarise(
    cp_value = dplyr::first(cp_value),
    n_rows = dplyr::n(),
    .groups = "drop"
  )

dupes_n <- sum(cp_panel$n_rows != 1)
cat("Country-year cells with duplicates (n_rows > 1): ", dupes_n, "\n\n")

if (any(cp_panel$n_rows != 1)) {
...
miss_by_country <- cp_panel |>
  ...
  dplyr::summarise(
    n_years = dplyr::n(),
    n_missing = sum(is.na(cp_value)),
    share_missing = mean(is.na(cp_value)),
    .groups = "drop"
  ) |>
...
readr::write_csv(miss_by_country, file.path(diag_dir, "cp_values_missingness_by_country.csv"))
readr::write_csv(coverage_by_year, file.path(diag_dir, "cp_values_country_coverage.csv"))
...
# =========================================
cat("\n=========================================\n")
cat("06_card_values_import_tidy.R — END\n")
cat("Timestamp: ", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n")
cat("=========================================\n\n")
