# scripts/21b_bis_ct_counts_import_tidy_with_CH.R
# =========================================
# Import & tidy BIS credit transfer counts (US + Canada + Switzerland, annual)
...
#   data/raw/bis/credit_transfers/counts/BIS_ct_counts_CH_annual_raw.csv
#
# Outputs (ROBUSTNESS PATH: includes CH):
#   data/intermediate/bis_with_CH/credit_transfers/ct_counts_country_year_intermediate_with_CH.csv
#   data/intermediate/bis_with_CH/credit_transfers/ct_counts_country_year_intermediate_with_CH.rds
#   outputs/tables/bis_ct_counts_key_coverage_with_CH.csv
#   outputs/tables/bis_ct_counts_missingness_by_country_with_CH.csv
#   outputs/tables/bis_ct_counts_country_coverage_with_CH.csv
#   outputs/tables/bis_ct_counts_series_metadata_with_CH.txt
#   outputs/logs/21b_bis_ct_counts_import_tidy_with_CH_log.txt
# =========================================
...
# 0. Logging
# =========================================
log_file <- here::here("outputs/logs", "21b_bis_ct_counts_import_tidy_with_CH_log.txt")
...
cat("\n=========================================\n")
cat("21b_bis_ct_counts_import_tidy_with_CH.R — START\n")
...
raw_dir <- here::here("data/raw/bis/credit_transfers/counts")

in_files <- stats::setNames(
  file.path(raw_dir, paste0("BIS_ct_counts_", unname(file_stub[controls]), "_annual_raw.csv")),
  controls
)
...
out_csv <- file.path(out_dir, "ct_counts_country_year_intermediate_with_CH.csv")
out_rds <- file.path(out_dir, "ct_counts_country_year_intermediate_with_CH.rds")

diag_dir <- here::here("outputs/tables")
dir.create(diag_dir, recursive = TRUE, showWarnings = FALSE)

diag_key_cov <- file.path(diag_dir, "bis_ct_counts_key_coverage_with_CH.csv")
diag_miss_country <- file.path(diag_dir, "bis_ct_counts_missingness_by_country_with_CH.csv")
diag_cov_year <- file.path(diag_dir, "bis_ct_counts_country_coverage_with_CH.csv")
meta_txt <- file.path(diag_dir, "bis_ct_counts_series_metadata_with_CH.txt")
...
tidy_list <- mapply(
  FUN = function(df, cc) tidy_bis_ct_counts(df, cc),
  df = raw_list,
  cc = names(raw_list),
  SIMPLIFY = FALSE
)
...
ct_counts <- dplyr::bind_rows(tidy_list) |>
...
if (nrow(dup) > 0) {
  ...
  print(head(dup, 20))
  stop("Non-unique (country,year) keys in BIS CT counts (with_CH).")
}
...
meta_lines <- c(
  "BIS CPMI Cashless (Credit transfers) — Counts (US + Canada + Switzerland) [with_CH]",
...
# =========================================
cat("\n=========================================\n")
cat("21b_bis_ct_counts_import_tidy_with_CH.R — END\n")
cat("Timestamp: ", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n")
cat("=========================================\n\n")
